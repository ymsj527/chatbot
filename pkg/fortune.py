...
from chat_tools import chatbot_tool
...
REDIS_URL = 'redis://:{}@192.168.1.14:60001/2'.format("firelark-redis@2024**")

class Master:
    def __init__(self):
        self.chatModel = ChatSparkLLM(
            model="gpt-3.5-turbo",
            temperature=0,
            streaming=True
        )
        ...
        tools = [chatbot_tool.search, chatbot_tool.get_info_from_local_db, chatbot_tool.bazi_cesuan, chatbot_tool.yaoyigua, chatbot_tool.jiemeng]
    ...
    def qingxu_chain(self, query: str):
        ...
        用户输入的内容是：{query}"""
        chain = ChatPromptTemplate.from_template(prompt) | ChatSparkLLM(temperature=0) | StrOutputParser()
